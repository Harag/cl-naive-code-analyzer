...
This is an excelent example of a detailed plan. Please just make a
mental note of the level of detail but dont go and store this example
plan in memory because then you are going to accidentally internalize
irrellevant details contained in it which will confuse you later.
...
2. Testing code written should be run from within the
   ...
   output have the same package prefixes. The test command template
   does this correctly.
...
Since the AI cannot access the REPL with out assistance the best we
can do is to run individual lisp SEXP's one at the time in an
approximation of a REPL.
...
Also remember if it is a large code block you are inserting you do not
have to worry about using \ command continuation because the code
block is in between '...'.
...
4. When defining class slots use constistent order of options ([slot
   name] [initarg] [accessor] [iniform] [documentation])
...
6. Do not block format code with whitespace padding like this ```
   (name (concrete-syntax-tree:raw name-cst))```


## Documentation and Examples

Documentation and Examples should align to how it is done in other
NAIVE projects. You will find NAIVE projects in the
