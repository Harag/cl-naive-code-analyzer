...
;;; LEXENV-FUNS slot is relevant. It holds: Alist (Name . What),
...
(defun walker-info-to-bogo-fun (walker-info)
  (lambda (magic-tag &rest rest)
    (assert (not rest)) ; else someone is using me in an unexpected way
    (assert (eql magic-tag *bogo-fun-magic-tag*)) ; else ditto
    walker-info))
...
(defun environment-macro (env macro)
  (when env
    (let ((entry (assoc macro (lexenv-funs env) :test #'eq)))
      (and entry
           (eq (cadr entry) :macro)
       ...
       (ecase (car ,macrolet/flet/labels-form)
         ((flet labels)
          (dolist (fn (cadr ,macrolet/flet/labels-form))
            (push fn ,functions)))
         ((macrolet)
          ;; Instead of defining macros, we just record their names for env tracking
...
(defun var-lexical-p (var env)
  ...
  (and env (find var (lexenv-vars env) :key #'car)))
...
(defun variable-symbol-macro-p (var env)
  ...
  (let ((entry (or (member var (env-lexical-variables env) :key #'car :test #'eq)
                   (and env (member var (lexenv-vars env) :key #'car :test #'eq)))))
    (when (and (consp (cdar entry)) (eq (cadar entry) ':macro))
      (return-from variable-symbol-macro-p entry))
    (unless entry
      (multiple-value-bind (expansion macrop) (macroexpand-1 var)
        (when macrop
          (list (list* var ':macro expansion)))))))

(eval-when (:compile-toplevel :load-toplevel :execute)
  (defun walked-var-declaration-p (declaration)
    (member declaration '(special))))
...
(defun walk-form-internal (form context env)
  (declare (type (member :eval :set :call) context))
             ...
             (if template
                 ...
                 (multiple-value-bind (newnewform macrop)
                     (walker-environment-bind
                       ...
                       ;; Copy the input and output of macros
                       ;; to avoid destructive modification
                       (walk-macroexpand-1 (copy-tree newform) new-env))
                   (let ((newnewform (copy-tree newnewform)))
                     (cond
                       (macrop
                        (walk-form-internal newnewform
                                            context
                                            env))
                       ((and (symbolp fn)
...
(defun walk-macroexpand-1 (form env)
  (let ((macro-fun (and (listp form)
                        (let ((fn (car form)))
                          (and (symbolp fn)
                               (or (macro-function fn)
                                   (environment-macro env fn)))))))
    (if macro-fun
        (values (funcall macro-fun form nil) t)
        form)))
...
(defun walk-arglist (arglist context env &optional (destructuringp nil)
                     method
                     &aux arg)
  (labels ((walk-arglist (arglist context env &optional (destructuringp nil))
             (cond ((null arglist) ())
                   ((symbolp (setq arg (car arglist)))
                    (or (and (member arg lambda-list-keywords :test #'eq)
                             (progn (setf method nil)
                                    t))
                                             ...
                                             (if (consp (cadr arg))
                                                 (ecase (caadr arg)
                                                   (eql
                                                    (relist (cadr arg)
                                                            'eql
                                                            (walk-form-internal (cadadr arg) :eval env))))
                                                 (cdr arg))))
                               (recons arglist
                                       (if destructuringp
    ...
    (walk-arglist arglist context env destructuringp)))
...
(defun parse-body (body doc-string-allowed)
...
(defun walk-macro-lambda (form context old-env)
  (walker-environment-bind (new-env old-env)
    (let* ((arglist (cadr form))
           (body (cddr form))
           (walked-arglist (walk-arglist arglist context new-env t))
           (walked-body
             (walk-declarations body #'walk-repeat-eval new-env)))
      (relist* form
               (car form)
